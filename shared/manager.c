...
static char BASE_DIR[PATH_MAX] = "account";
static char GENERATOR_BIN[PATH_MAX] = "./generator";
...
static void resolve_paths(void) {
    /* Always use account directory one level up, as requested */
    strcpy(BASE_DIR, "../account");

    /* Find generator binary - try several common locations */
    if (path_exists("./generator") && access("./generator", X_OK) == 0) {
        strcpy(GENERATOR_BIN, "./generator");
    } else if (path_exists("./shared/generator") && access("./shared/generator", X_OK) == 0) {
        strcpy(GENERATOR_BIN, "./shared/generator");
    } else if (path_exists("../shared/generator") && access("../shared/generator", X_OK) == 0) {
        strcpy(GENERATOR_BIN, "../shared/generator");
    } else {
        strcpy(GENERATOR_BIN, "./generator");
...
static int resolve_plain_dir(const char *name, char *dir_out) {
    ...
    if (is_uuid_format(name)) return 0;
...
static int cmd_remote_register(const char *host, int port, const char *name, const char *key) {
    int fd;
    ...
    char req[512];
    snprintf(req, sizeof(req), "remote_register %s %s\n", name, key);
    ...
    if (r <= 0) return 1;
    resp[r] = '\0';
    if (strstr(resp, "OK: registered") != NULL) {
        printf("Remote registration successful at %s:%d\n", host, port);
        return 0;
    }
    printf("Remote registration failed: %s", resp);
    return 1;
}
...
static void print_usage(void) {
    ...
    fprintf(stderr, "  manager remote <host> <port> <name> <key>\n");
...
int main(int argc, char **argv) {
    ...
    if (strcmp(*(argv + 1), "remote") == 0) {
        ...
        return cmd_remote_register(*(argv + 2), atoi(*(argv + 3)), *(argv + 4), *(argv + 5));
    }
