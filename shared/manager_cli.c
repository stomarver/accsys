...
static int call_manager(char *args[]) {
    char mbin[PATH_MAX];
    if (path_exists("./manager") && access("./manager", X_OK) == 0) strcpy(mbin, "./manager");
    else if (path_exists("./shared/manager") && access("./shared/manager", X_OK) == 0) strcpy(mbin, "./shared/manager");
    else if (path_exists("../shared/manager") && access("../shared/manager", X_OK) == 0) strcpy(mbin, "../shared/manager");
    else strcpy(mbin, "./manager");
    args[0] = mbin;
    pid_t pid;
    int status;
...
static int backend_remote(const char *host, const char *port, const char *name, const char *key) {
    char *args[] = { MANAGER_BIN, "remote", (char *)host, (char *)port, (char *)name, (char *)key, NULL };
    return call_manager(args);
}
...
static void render_menu_frame(const char *input, int cursor_on) {
    ...
    if (use_color) {
        int fixed = 42;  /* " ctrl + register / rename / rekey / remote" */
        pad = term_cols - fixed;
        if (pad < 0) pad = 0;
        ...
        fputs(ANSI_RREV, stdout); putchar('g'); fputs(ANSI_RST ANSI_REV, stdout);
        fputs("ister / re", stdout);

        fputs(ANSI_RREV, stdout); putchar('n'); fputs(ANSI_RST ANSI_REV, stdout);
        fputs("ame / re", stdout);

        fputs(ANSI_RREV, stdout); putchar('k'); fputs(ANSI_RST ANSI_REV, stdout);
        fputs("ey / re", stdout);

        fputs(ANSI_RREV, stdout); putchar('m'); fputs(ANSI_RST ANSI_REV, stdout);
        fputs("ote", stdout);

        for (i = 0; i < pad; i++) putchar(' ');
        fputs(ANSI_RST, stdout);
    } else {
        print_inverted_full(" ctrl + register / rename / rekey / remote");
    }
    ...
    for (int row = 4; row < term_rows; row++) {
        move_cursor(row, 1);
        fputs(ANSI_CL, stdout);
    }
    ...
    *buf = 0;
    ...
    while (1) {
        ...
        while (input_available()) {
            ...
            if (ch == CTRL_R) { strcpy(buf, "remote");   return 1; }
...
static int interactive_mode(void) {
    ...
    while (run) {
        ...
        if (strcmp(choice, "remote") == 0) {
            char host[INPUT_BUF_SZ];
            char port[INPUT_BUF_SZ];
            r = prompt_line("remote", "server host (default 127.0.0.1)", host, sizeof(host));
            if (r <= 0) { run = 0; continue; }
            if (host[0] == '\0') strcpy(host, "127.0.0.1");
            r = prompt_line("remote", "server port (default 9000)", port, sizeof(port));
            if (r <= 0) { run = 0; continue; }
            if (port[0] == '\0') strcpy(port, "9000");
            r = prompt_line("remote", "name", name, sizeof(name));
            if (r <= 0) { run = 0; continue; }
            r = prompt_line("remote", "key", key, sizeof(key));
            if (r <= 0) { run = 0; continue; }
            show_result("remote", NULL, backend_remote(host, port, name, key));
            continue;
        }

        /* unknown command -> just loop */
    }
