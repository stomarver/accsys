...
static int call_manager(char *args[]) {
    ...
    char mbin[PATH_MAX];
    if (in_sub && path_exists("../shared/manager") && access("../shared/manager", X_OK) == 0) strcpy(mbin, "../shared/manager");
...
static void render_menu_frame(const char *input, int cursor_on) {
    ...
    if (use_color) {
        ...
        fputs(ANSI_RREV, stdout); putchar('m');
        fputs(ANSI_RST ANSI_REV, stdout);
        fputs("ote", stdout);
...
static int interactive_mode(void) {
    ...
    while (run) {
        ...
        if (strcmp(choice, "remote") == 0 || strcmp(choice, "4") == 0) {
