...
static int auth_admin(const char *key) {
    char key_file[PATH_MAX];
    char stored_key[MAX_LINE];
    if (!generator_available()) return 1;
    snprintf(key_file, sizeof(key_file), "%s/admin/key", ACCOUNT_DIR);
    if (!read_file_line(key_file, stored_key, sizeof(stored_key))) return 1;
...
static int auth(const char *identifier, const char *key) {
    ...
    if (strcmp(identifier, "admin") == 0) return auth_admin(key);
    ...
    fprintf(stderr, "auth failed: bad name or key\n");
    return 1;
}
