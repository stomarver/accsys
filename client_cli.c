    ...
    while (running && client_fd != -1) {
        ...
        if (r > 0 && FD_ISSET(STDIN_FILENO, &rfds)) {
            ...
            if (ch == '\n' || ch == '\r') {
                ...
                if (strncmp(input, "msg ", 4) == 0 || strcmp(input, "msg") == 0) {
                    char msg_buf[512];
                    const char *msg_text = "";
                    if (strncmp(input, "msg ", 4) == 0) {
                        msg_text = input + 4;
                    }
                    snprintf(msg_buf, sizeof(msg_buf), "msg %s\n", msg_text);
                    if (send_server(msg_buf) == 0) {
                        show_result("client", "msg sent", 0);
                    } else {
                        show_result("client", "msg failed", 1);
                    }
                    pos = 0; *input = 0;
                    clear_screen();
                    continue;
