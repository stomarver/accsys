...
static void render_status_shell(const char *input, int cursor_on) {
    ...
    uptime = (long)(time(NULL) - client_connected);
    snprintf(status, sizeof(status), " %s:%d  %s  uptime %lds",
             client_host, client_port, client_name, uptime);

    snprintf(header, sizeof(header), " client > %s", status);
    render_header(header);
    clear_line(2);
    move_cursor(3, 1);
    fputs(" > ", stdout);
    ...
    move_cursor(term_rows, 1);
    fputs(ANSI_REV, stdout);
    fputs(" ping / logout / quit ", stdout);
    int pad = term_cols - 22;
    if (pad < 0) pad = 0;
    while (pad-- > 0) putchar(' ');
    fputs(ANSI_RST, stdout);
    fflush(stdout);
    ...
    *input = 0;
    ...
    while (running && client_fd != -1) {
        ...
        if (r > 0 && FD_ISSET(client_fd, &rfds)) {
            ...
            buf[r] = '\0';
            /* server responses (pong) are handled silently; we could show them */
        }
